...
- [ ] Broaden ray/interface invalid corpus (mixed storage-class/ray-model mismatches) with shrink-friendly `Arbitrary` implementations and add parity checks when the C++ fuzz bridge becomes available. (Progress: added workgroup/output/mixed IO, private/function/cross-workgroup/generic, uniform-constant/push-constant, uniform/storage-buffer, shader-record buffer, task payload, atomic counter, image, physical storage buffer, code-section, device-only, and input/output mix interface invalids; parity checks still blocked on the C++ fuzz bridge.)
