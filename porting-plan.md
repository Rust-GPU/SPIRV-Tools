...
- Add paired text/binary regressions for version/extension/capability gating, covering env-clamped cases and Vulkan/OpenCL/WebGPU allowlists.
  - Added operand-version regressions for `ExecutionModeId` and memory-semantics `MakeVisible`, and taught the Rust loader path to retain `OpExecutionModeId` when parsing binaries.
