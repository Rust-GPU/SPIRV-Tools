...
- [ ] Broaden ray/interface invalid corpus (mixed storage-class/ray-model mismatches) with shrink-friendly `Arbitrary` implementations and add parity checks when the C++ fuzz bridge becomes available. (Progress: added workgroup/output/mixed IO, private/function/cross-workgroup/generic, uniform-constant/push-constant, uniform/storage-buffer, shader-record buffer, task payload, atomic counter, and image interface invalids; parity checks still blocked on the C++ fuzz bridge.)
