...
- Add more affine/distributivity rewrites to expose constant folding (mixed add/sub chains, constant factorization) and port matching C++ arithmetic tests to Rust/FFI/CLI harnesses. (added add/sub operand cancellation, constant-chain merges, shared-addend cancellation including constant addends `(x+c)-(y+c)=>x-y`, commuted shared addends `(b+a)-(d+b)=>a-d`, chained sub merges `(x-y)+(y-z)=>x-z`, and mirrored add/sub cancel `(x)+(y-x)=>y` with unit coverage; broader factorization still pending)
