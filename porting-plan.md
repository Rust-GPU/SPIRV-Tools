...
- Add more affine/distributivity rewrites to expose constant folding (mixed add/sub chains, constant factorization) and port matching C++ arithmetic tests to Rust/FFI/CLI harnesses. (added add/sub operand cancellation, constant-chain merges, and shared-addend cancellation including constant addends `(x+c)-(y+c)=>x-y` and commuted shared addends `(b+a)-(d+b)=>a-d` with unit coverage; broader factorization still pending)
