...
- Added logical xnor rewrites so `(a || !b) && (!a || b)` folds to `leq(a, b)`, with Rust unit coverage.
- Added logical factoring rewrites so `(a && b) || (a && c)` becomes `a && (b || c)`, with Rust unit coverage.
- Added logical factoring rewrites so `(a || b) && (a || c)` becomes `a || (b && c)`, with Rust unit coverage.
- Added logical xor rewrites so `(a || b) && (!a || !b)` folds to `lne(a, b)`, with Rust unit coverage.
- Added logical factoring rewrites so `(a || b) || (a || c)` becomes `a || (b || c)`, with Rust unit coverage.
- Added logical factoring rewrites so `(a && b) && (a && c)` becomes `a && (b && c)`, with Rust unit coverage.
...
- Rust-only: added logical ne absorption so `a && (a != b)` folds to `a && !b`, with Rust unit coverage.
- Rust-only: added logical eq absorption so `!a && (a == b)` folds to `!a && !b`, with Rust unit coverage.
- Rust-only: added logical ne absorption so `!a && (a != b)` folds to `!a && b`, with Rust unit coverage.
- Rust-only: added logical eq absorption so `a || (a == b)` folds to `a || !b`, with Rust unit coverage.
- Rust-only: added logical ne absorption so `a || (a != b)` folds to `a || b`, with Rust unit coverage.
...
- Rust-only: added logical eq absorption so `!a || (b == a)` folds to `!a || b`, with Rust unit coverage.
- Rust-only: added logical ne absorption so `!a || (b != a)` folds to `!a || !b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `a == (a && b)` folds to `!a || b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `a != (a && b)` folds to `a && !b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `a == (a || b)` folds to `a || !b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `a != (a || b)` folds to `!a && b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `a == (!a && b)` folds to `!a && b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `a != (!a && b)` folds to `a || !b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `a == (!a || b)` folds to `a && b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `a != (!a || b)` folds to `!a || !b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `(a && b) == (a && !b)` folds to `!a`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `(a && b) != (a && !b)` folds to `a`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `(a && b) == (!a && b)` folds to `!b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `(a && b) != (!a && b)` folds to `b`, with Rust unit coverage.
...
- Rust-only: added logical eq rewrite so `(a || !b) == (!a || b)` folds to `a == b`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `(a || !b) != (!a || b)` folds to `a != b`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `(a && b) == (!a || !b)` folds to `false`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `(a && b) != (!a || !b)` folds to `true`, with Rust unit coverage.
- Rust-only: added logical eq rewrite so `(a || b) == (!a && !b)` folds to `false`, with Rust unit coverage.
- Rust-only: added logical ne rewrite so `(a || b) != (!a && !b)` folds to `true`, with Rust unit coverage.
...
- Rust-only: added bitwise xnor absorption so `~y & ~(x ^ y)` folds to `~x & ~y`, with Rust unit coverage.
- Rust-only: added bitwise xnor absorption so `x | ~(x ^ y)` folds to `x | ~y`, with Rust unit coverage.
- Rust-only: added bitwise xnor absorption so `y | ~(x ^ y)` folds to `y | ~x`, with Rust unit coverage.
- Rust-only: added bitwise xnor absorption so `~x | ~(x ^ y)` folds to `~x | y`, with Rust unit coverage.
- Rust-only: added bitwise xnor absorption so `~y | ~(x ^ y)` folds to `~y | x`, with Rust unit coverage.
...
- Added eq/ne rewrites that cancel odd constant multipliers in comparisons, with Rust unit coverage.
...
- Added eq/ne rewrites that reduce `x ^ y == ~x` to `y == all_ones`, with Rust unit coverage.
- Added eq/ne rewrites that reduce `x ^ y == ~y` to `x == all_ones`, with Rust unit coverage.
- Added eq/ne rewrites that reduce `x + y == x` to `y == 0` using width-aware zero constants, with Rust unit coverage.
- Added eq/ne rewrites that reduce `x - y == x` to `y == 0` using width-aware zero constants, with Rust unit coverage.
- Added eq/ne rewrites that reduce `x + y == 0` to `x == -y`, with Rust unit coverage.
...
- Added bitwise absorption rewrites so `(~x) | (x & y)` reduces to `(~x) | y`, with Rust unit coverage.
- Added eq/ne rewrites that fold `x == ~x` to false and `x != ~x` to true, with Rust unit coverage.
...
- Added bxor rewrites so `(~x) ^ (~x & y)` folds to `~(x | y)`, with Rust unit coverage.
- Added bxor rewrites so `(~x) ^ (~x | y)` folds to `(x & y)`, with Rust unit coverage.
- Added eq/ne rewrites that move bitwise-not across comparisons (`~x == y` -> `x == ~y`), with Rust unit coverage.
- Added eq/ne rewrites that move negation across comparisons (`-x == y` -> `x == -y`), with Rust unit coverage.
...
- Added bitwise rewrites so `(x | y) & ~(x & y)` collapses to `x ^ y`, with Rust unit coverage.
- Added bitwise rewrites so `(x & y) | ~(x | y)` collapses to `~(x ^ y)`, with Rust unit coverage.
- Added bitwise rewrites so `(x & ~y) | (~x & y)` folds to `x ^ y`, with Rust unit coverage.
- Added bitwise rewrites so `(x | ~y) & (~x | y)` folds to `~(x ^ y)`, with Rust unit coverage.
- Added bitwise rewrites so `(x | y) ^ (x | ~y)` folds to `~x`, with Rust unit coverage.
- Added bitwise rewrites so `(x | y) ^ (~x | y)` folds to `~y`, with Rust unit coverage.
- Added bitwise rewrites so `(x | y) ^ (x & ~y)` folds to `y`, with Rust unit coverage.
- Added bitwise rewrites so `(x | y) ^ (~x & y)` folds to `x`, with Rust unit coverage.
- Added bitwise rewrites so `(x & y) ^ (~x | y)` folds to `~x`, with Rust unit coverage.
- Added bitwise rewrites so `(x & y) ^ (x | ~y)` folds to `~y`, with Rust unit coverage.
- Added bitwise rewrites so `(x & y) ^ (~x & ~y)` folds to `~(x ^ y)`, with Rust unit coverage.
- Added bitwise rewrites so `(x ^ y) & ~y` folds to `x & ~y`, with Rust unit coverage.
- Added bitwise rewrites so `(x ^ y) & y` folds to `~x & y`, with Rust unit coverage.
- Added bitwise rewrites so `(x ^ y) | (x | y)` folds to `x | y`, with Rust unit coverage.
...
- Added bitwise rewrites so `~(x ^ y) & ~(x | y)` folds to `~(x | y)`, with Rust unit coverage.
- Added bitwise rewrites so `~(x ^ y) | ~(x | y)` folds to `~(x ^ y)`, with Rust unit coverage.
- Added bitwise rewrites so `(x ^ y) & ~(x | y)` folds to zero, with Rust unit coverage.
- Added bitwise rewrites so `(x ^ y) | ~(x | y)` folds to `~(x & y)`, with Rust unit coverage.
- Added bxor rewrites so `(x ^ y) ^ (x & y)` folds to `x | y`, with Rust unit coverage.
...
- Added bxor rewrites so `x ^ ~(x | y)` folds to `x | ~y`, with Rust unit coverage.
- Added bxor rewrites so `x ^ ~(x & y)` folds to `~x | y`, with Rust unit coverage.
- Added bxor rewrites that cancel nested operands (`x ^ (x ^ y)`), with Rust unit coverage.
- Added bxor rewrites so `~x ^ (x ^ y)` folds to `~y`, with Rust unit coverage.
- Added bxor rewrites so `~x ^ ~(x ^ y)` folds to `y`, with Rust unit coverage.
...
- Added unsigned compare rewrite so `ult(0, x)` folds to `x != 0`, with Rust unit coverage.
- Added unsigned compare rewrite so `ult(x, max)` folds to `x != max`, with Rust unit coverage.
- Added unsigned compare rewrite so `ult(max, x)` folds to `false`, with Rust unit coverage.
- Added unsigned compare rewrite so `ule(x, 0)` folds to `x == 0`, with Rust unit coverage.
- Added unsigned compare rewrite so `ule(0, x)` folds to `true`, with Rust unit coverage.
- Added unsigned compare rewrite so `ule(x, max)` folds to `true`, with Rust unit coverage.
- Added unsigned compare rewrite so `ule(max, x)` folds to `x == max`, with Rust unit coverage.
- Added unsigned compare rewrite so `ugt(x, 0)` folds to `x != 0`, with Rust unit coverage.
- Added unsigned compare rewrite so `ugt(0, x)` folds to `false`, with Rust unit coverage.
- Added unsigned compare rewrite so `ugt(x, max)` folds to `false`, with Rust unit coverage.
- Added unsigned compare rewrite so `ugt(max, x)` folds to `x != max`, with Rust unit coverage.
- Added unsigned compare rewrite so `uge(x, 0)` folds to `true`, with Rust unit coverage.
- Added unsigned compare rewrite so `uge(0, x)` folds to `x == 0`, with Rust unit coverage.
- Added unsigned compare rewrite so `uge(x, max)` folds to `x == max`, with Rust unit coverage.
- Added unsigned compare rewrite so `uge(max, x)` folds to `true`, with Rust unit coverage.
- Added signed compare rewrite so `slt(x, min)` folds to `false`, with Rust unit coverage.
- Added signed compare rewrite so `slt(min, x)` folds to `x != min`, with Rust unit coverage.
- Added signed compare rewrite so `slt(x, max)` folds to `x != max`, with Rust unit coverage.
- Added signed compare rewrite so `slt(max, x)` folds to `false`, with Rust unit coverage.
...
- Added signed compare rewrite so `sge(x, min)` folds to `true`, with Rust unit coverage.
...
- Added signed compare rewrite so `slt(x, min+1)` folds to `x == min`, with Rust unit coverage.
- Added signed compare rewrite so `sge(x, min+1)` folds to `x != min`, with Rust unit coverage.
- Added signed compare rewrite so `sgt(x, max-1)` folds to `x == max`, with Rust unit coverage.
...
- Added select nesting rewrite so `select(c, select(c, x, y), z)` folds to `select(c, x, z)`, with Rust unit coverage.
- Added select nesting rewrite so `select(c, select(c, x, y), select(c, z, w))` folds to `select(c, x, w)`, with Rust unit coverage.
- Added select nesting rewrite so `select(c, x, select(!c, y, z))` folds to `select(c, x, y)`, with Rust unit coverage.
- Added bitwise AND commutation rewrite so `x & y` can reorder operands, with Rust unit coverage.
...
- [x] Run arithmetic + control rewrites in a single per-function e-graph pass, with dominator-aware extraction (nearest-dominator placement + loop-preheader hoists) to keep id stability and global optimization.
- Run all rewrites (arithmetic + control + CFG) in a single e-graph per function, using shared roots for control-flow patterns so optimizations interact globally; rely on extraction/codegen for placement instead of post-pass CSE/hoists. (CFG rewrites pending)
...
- [x] Normalize logical comparisons via e-graph rewrites (commutativity, `logical eq/ne` with constants, `!cmp` inversion to opposite predicates, and `x && !x` / `x || !x` folds) with Rust unit coverage.
...
- [x] Reducer/fuzzer corpus parity and smoke suites that mirror the legacy C++ tool behavior when invoked through `spirv-reduce`/`spirv-fuzz` binaries. (CLI corpus now covers vertex/fragment/compute plus raygen/miss/closest-hit/callable entry points with skip-on-missing C++ tooling)
...
- Added select nesting rewrite for mixed negated/positive inner conditions (`select c (select c x y) (select !c z w) -> select c x z`) with Rust coverage.
...
- Added shift factoring rewrite for OR with shared left shift (`(x << c) | (y << c) -> (x | y) << c`) with Rust coverage.
- Added shift factoring rewrite for XOR with shared left shift (`(x << c) ^ (y << c) -> (x ^ y) << c`) with Rust coverage.
- Added shift factoring rewrite for AND with shared logical right shift (`(x >>u c) & (y >>u c) -> (x & y) >>u c`) with Rust coverage.
...
- Added nested absorption rewrite for AND over OR with outer commutation (`(y | (x & z)) & x -> x & (y | z)`) with Rust coverage.
...
- Added nested absorption rewrite for OR over AND with outer commutation (`(y & (x | z)) | x -> x | (y & z)`) with Rust coverage.
...
- Added nested complement absorption for inverted AND (`~x & (y | (x & z)) -> ~x & y`) with Rust coverage.
- Added nested complement absorption for inverted OR (`~x | (y & (x | z)) -> ~x | y`) with Rust coverage.
- Added nested absorption rewrite for logical AND over OR (`a && (b || (a && c)) -> a && (b || c)`) with Rust coverage.
- Added nested absorption rewrite for logical AND over OR with commuted AND (`a && (b || (c && a)) -> a && (b || c)`) with Rust coverage.
- Added nested absorption rewrite for logical OR over AND (`a || (b && (a || c)) -> a || (b && c)`) with Rust coverage.
- Added nested absorption rewrite for logical OR over AND with commuted OR (`a || (b && (c || a)) -> a || (b && c)`) with Rust coverage.
- Added nested complement absorption for logical AND over OR (`a && (b || (!a && c)) -> a && b`) with Rust coverage.
- Added nested complement absorption for logical AND over OR with commuted AND (`a && (b || (c && !a)) -> a && b`) with Rust coverage.
