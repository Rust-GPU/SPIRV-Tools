...
- Document the Rust-first fuzz path and mark the C++ bridge as deprecated/disabled by default, including rollout/rollback knobs for downstream consumers.
 - Seeded Rust path now uses an `arbitrary`-driven structured generator with typed validity markers (Valid/Unchecked/IntentionallyInvalid) and invalid mutations (missing memory model/terminator); expand to richer transformations and corpus coverage next.
