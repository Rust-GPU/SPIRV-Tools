...
- [ ] Broaden ray/interface invalid corpus (mixed storage-class/ray-model mismatches) with shrink-friendly `Arbitrary` implementations and add parity checks when the C++ fuzz bridge becomes available. (Progress: added workgroup/output/mixed IO interface invalids; parity checks still blocked on the C++ fuzz bridge.)
