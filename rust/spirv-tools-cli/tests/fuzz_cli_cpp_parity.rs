...
fn simple_module_text() -> &'static str {
    "\
OpCapability Shader
...
fn spirv_fuzz_cli_matches_cpp_output_when_available() {
    ...
    let input_spv = dir.path().join("module.spv");
    fs::write(&asm_path, simple_module_text()).expect("write asm");
    ...
    assert!(rust_status.success(), "rust spirv-fuzz failed: {rust_status:?}");
    assert!(cpp_status.success(), "cpp spirv-fuzz failed: {cpp_status:?}");
