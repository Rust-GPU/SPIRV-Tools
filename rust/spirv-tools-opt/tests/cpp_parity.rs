...
#[test]
fn rust_and_cpp_arith_outputs_match_sdiv_pow2_wide() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_sdiv_pow2_64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for wide signed div by pow2"
    );
}
...
#[test]
fn rust_and_cpp_arith_outputs_match_signed_width_sized_mask_then_shift() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_signed_mask_then_shift_width_sized_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed width-sized mask then shift"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_width_sized_mask_then_shift() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_mask_then_shift_width_sized_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for width-sized mask then shift"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_mask_then_shift_u64() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_mask_then_shift_u64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for 64-bit mask then shift"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_signed_mask_then_shift_u64() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_signed_mask_then_shift_u64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed 64-bit mask then shift"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_preserve_full_mask_band() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_band_full_mask_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for full-mask bitand"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_bor_folding() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_bor_fold_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
...
#[test]
fn rust_and_cpp_arith_outputs_match_bxor_complement_folding() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let module_words = build_bxor_complement_fold_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
...
#[test]
fn rust_and_cpp_arith_outputs_match_bnot_folding() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let module_words = build_bnot_fold_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for bitwise NOT folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for rotate folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_commuted() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_commuted_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for rotate folding (commuted)"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_signed() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_signed_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed rotate folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_signed_commuted() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_signed_commuted_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed rotate folding (commuted)"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_u64() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_u64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for 64-bit rotate folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_u64_commuted() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_u64_commuted_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for 64-bit rotate folding (commuted)"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_signed_u64() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_signed_u64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed 64-bit rotate folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_rotate_fold_signed_u64_commuted() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_rotate_fold_signed_u64_commuted_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for signed 64-bit rotate folding (commuted)"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_band_complement_fold_u64() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_band_complement_fold_u64_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for 64-bit band complement folding"
    );
}

#[test]
fn rust_and_cpp_arith_outputs_match_band_complement_fold_u32() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };
    let module_words = build_band_complement_fold_u32_module();
    let rust_sig = arith_signature(
        &spirv_tools_opt::translate::optimize_arith_block(&extract_arith_insts(&module_words))
            .expect("rust optimize"),
    );
    ...
    assert_eq!(
        rust_sig, cpp_sig,
        "Rust vs C++ arithmetic output mismatch for 32-bit band complement folding"
    );
}
...
#[test]
fn rust_and_cpp_factor_mixed_unit_add() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mixed_unit_factor_add_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const4_ids: std::collections::HashSet<u32> = rust_optimized
        .iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32(4)]
    ...
    let rust_mul_matches = rust_optimized.iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && rust_const4_ids.contains(b))
                        || (b == &x_id && rust_const4_ids.contains(a))
            )
    });
    assert!(
        rust_mul_matches,
        "rust optimizer should fold x + (x*3) to 4*x"
    );
    ...
    parse_words(&cpp_words, &mut loader).expect("parse cpp optimized");
    let module = loader.module();
    let cpp_const4_ids: std::collections::HashSet<u32> = module
        .all_inst_iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32(4)]
    ...
    let cpp_mul_matches = module.all_inst_iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && cpp_const4_ids.contains(b))
                        || (b == &x_id && cpp_const4_ids.contains(a))
            )
    });
    assert!(
        cpp_mul_matches,
        "C++ spirv-opt should factor x + (x*3) to 4*x"
    );
}

#[test]
fn rust_and_cpp_factor_mixed_unit_sub() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mixed_unit_factor_sub_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const6_ids: std::collections::HashSet<u32> = rust_optimized
        .iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
    ...
    let rust_mul_matches = rust_optimized.iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && rust_const6_ids.contains(b))
                        || (b == &x_id && rust_const6_ids.contains(a))
            )
    });
    assert!(
        rust_mul_matches,
        "rust optimizer should fold (7*x) - x to 6*x"
    );
    ...
    parse_words(&cpp_words, &mut loader).expect("parse cpp optimized");
    let module = loader.module();
    let cpp_const6_ids: std::collections::HashSet<u32> = module
        .all_inst_iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32(6)]
    ...
    let cpp_mul_matches = module.all_inst_iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && cpp_const6_ids.contains(b))
                        || (b == &x_id && cpp_const6_ids.contains(a))
            )
    });
    assert!(
        cpp_mul_matches,
        "C++ spirv-opt should factor (7*x) - x to 6*x"
    );
}
...
#[test]
fn rust_and_cpp_reduce_const_ratio_in_division() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mul_div_constant_ratio_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const2_ids: std::collections::HashSet<u32> = rust_optimized
        .iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32(2)]
        })
        .filter_map(|inst| inst.result_id)
        .collect();
    let rust_mul_matches = rust_optimized.iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && rust_const2_ids.contains(b))
                        || (b == &x_id && rust_const2_ids.contains(a))
            )
    });
    let rust_has_div = rust_optimized
        .iter()
        .any(|inst| matches!(inst.class.opcode, Op::SDiv | Op::UDiv));
    assert!(
        rust_mul_matches && !rust_has_div,
        "rust optimizer should turn (6*x)/3 into 2*x without a division"
    );
    ...
    parse_words(&cpp_words, &mut loader).expect("parse cpp optimized");
    let module = loader.module();
    let cpp_const2_ids: std::collections::HashSet<u32> = module
        .all_inst_iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32(2)]
    ...
    let cpp_mul_matches = module.all_inst_iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && cpp_const2_ids.contains(b))
                        || (b == &x_id && cpp_const2_ids.contains(a))
            )
    });
    let cpp_has_div = module
        .all_inst_iter()
        .any(|inst| matches!(inst.class.opcode, Op::SDiv | Op::UDiv));
    assert!(
        cpp_mul_matches && !cpp_has_div,
        "C++ spirv-opt should turn (6*x)/3 into 2*x without a division"
    );
}

#[test]
fn rust_and_cpp_reduce_signed_const_ratio_in_division() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mul_div_constant_ratio_signed_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const2_ids: std::collections::HashSet<u32> = rust_optimized
...
#[test]
fn rust_and_cpp_reduce_signed_const_ratio_mixed_sign_dividend() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mul_div_constant_ratio_mixed_sign_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_neg2_ids: std::collections::HashSet<u32> = rust_optimized
        .iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32((-2i32) as u32)]
        })
        .filter_map(|inst| inst.result_id)
        .collect();
    let rust_mul_matches = rust_optimized.iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && rust_const_neg2_ids.contains(b))
                        || (b == &x_id && rust_const_neg2_ids.contains(a))
            )
    });
    let rust_has_div = rust_optimized
        .iter()
    ...
    parse_words(&cpp_words, &mut loader).expect("parse cpp optimized");
    let module = loader.module();
    let cpp_const_neg2_ids: std::collections::HashSet<u32> = module
        .all_inst_iter()
        .filter(|inst| {
            inst.class.opcode == Op::Constant
                && inst.operands == vec![rspirv::dr::Operand::LiteralBit32((-2i32) as u32)]
    ...
    let cpp_mul_matches = module.all_inst_iter().any(|inst| {
        inst.class.opcode == Op::IMul
            ...
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
                    if (a == &x_id && cpp_const_neg2_ids.contains(b))
                        || (b == &x_id && cpp_const_neg2_ids.contains(a))
            )
    });
    let cpp_has_div = module
        .all_inst_iter()
...
#[test]
fn rust_and_cpp_reduce_signed_const_ratio_positive_dividend_negative_divisor() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id, x_id) = build_mul_div_constant_ratio_pos_neg_div_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_neg2_ids: std::collections::HashSet<u32> = rust_optimized
...
#[test]
fn rust_and_cpp_preserve_signed_non_divisible_ratio_mixed_sign() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_div_non_divisible_ratio_mixed_sign_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    ...
    assert!(
        rust_div_matches,
        "rust optimizer should keep (-5*x)/3 as a division when constants are not divisible"
    );
    ...
    assert!(
        cpp_div_matches,
        "C++ spirv-opt should keep (-5*x)/3 as a division when constants are not divisible"
    );
}

#[test]
fn rust_and_cpp_preserve_signed_non_divisible_ratio_pos_neg_divisor() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_div_non_divisible_ratio_pos_neg_div_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_div_matches = rust_optimized
...
fn rust_and_cpp_merge_nested_const_divisors() {
    ...
    let (module_words, result_id, x_id) = build_nested_const_div_merge_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const6_ids: std::collections::HashSet<u32> = rust_optimized
        ...
        .filter_map(|inst| inst.result_id)
        .collect();
    let rust_div_matches = rust_optimized.iter().any(|inst| {
        inst.class.opcode == Op::UDiv
            && inst.result_id == Some(result_id)
            && matches!(
                inst.operands.as_slice(),
                [rspirv::dr::Operand::IdRef(a), rspirv::dr::Operand::IdRef(b)]
    ...
    assert!(
        rust_div_matches,
        "rust optimizer should merge ((x/2)/3) into x/6"
    );
    ...
    let cpp_const6_ids: std::collections::HashSet<u32> = module
        ...
        .filter_map(|inst| inst.result_id)
        .collect();
    let cpp_div_matches = module.all_inst_iter().any(|inst| {
        inst.class.opcode == Op::UDiv
            && inst.result_id == Some(result_id)
            && matches!(
                inst.operands.as_slice(),
    ...
    assert!(
        cpp_div_matches,
        "C++ spirv-opt should merge ((x/2)/3) into x/6"
    );
}

#[test]
fn rust_and_cpp_fold_mul_mod_same_const_to_zero() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_mod_same_const_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    ...
    let rust_has_mod = rust_optimized
        .iter()
        .any(|inst| matches!(inst.class.opcode, Op::UMod | Op::SRem));
    assert!(
        rust_const_zero && !rust_has_mod,
        "rust optimizer should fold (x*5) % 5 to zero"
    );
    ...
    let cpp_has_mod = module
        .all_inst_iter()
        .any(|inst| matches!(inst.class.opcode, Op::UMod | Op::SRem));
    assert!(
        cpp_const_zero && !cpp_has_mod,
        "C++ spirv-opt should fold (x*5) % 5 to zero"
    );
}

#[test]
fn rust_and_cpp_fold_mul_mod_divisible_const_to_zero() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_mod_divisible_const_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_preserve_mul_mod_non_divisible_const() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_mod_non_divisible_const_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
    ...
    let cpp_has_mod = module
        ...
        .any(|inst| matches!(inst.class.opcode, Op::UMod | Op::SRem));
    assert!(
        cpp_has_mod && !cpp_const_zero,
        "C++ spirv-opt should not fold (x*5) % 3 to zero"
    );
}

#[test]
fn rust_and_cpp_fold_mul_srem_divisible_const_to_zero() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_divisible_const_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_fold_mul_srem_divisible_const_positive_to_zero() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_divisible_const_pos_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_preserve_mul_srem_non_divisible_const() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_non_divisible_const_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_preserve_mul_srem_non_divisible_const_positive() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_non_divisible_const_pos_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_fold_mul_srem_divisible_const_mixed_sign_to_zero() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_divisible_const_mixed_sign_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
#[test]
fn rust_and_cpp_preserve_mul_srem_non_divisible_const_mixed_sign() {
    let Some(cpp_opt) = cpp_opt_bin() else {
        return;
    };

    let (module_words, result_id) = build_mul_srem_non_divisible_const_mixed_sign_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_const_zero = rust_optimized.iter().any(|inst| {
...
fn rust_and_cpp_preserve_non_divisible_const_ratio_in_division() {
    ...
    let (module_words, result_id) = build_mul_div_non_divisible_ratio_module();
    let rust_insts = extract_simple_block(&module_words);
    let rust_optimized =
        spirv_tools_opt::translate::optimize_arith_block(&rust_insts).expect("rust optimizer");
    let rust_udiv_matches = rust_optimized
        .iter()
        .any(|inst| inst.class.opcode == Op::UDiv && inst.result_id == Some(result_id));
    assert!(
        rust_udiv_matches,
        "rust optimizer should keep (5*x)/2 as a division when constants are not divisible"
    );
...
fn build_sdiv_pow2_64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    let int = b.type_int(64, 1); // signed 64-bit
    let func_ty = b.type_function(void, vec![]);
    let _ = b
        .begin_function(void, None, FunctionControl::NONE, func_ty)
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.variable(int, None, rspirv::spirv::StorageClass::Function, None);
    let pow2 = b.constant_bit64(int, 1u64 << 33);
    let div = b.s_div(int, None, x, pow2).expect("sdiv");
    let _ = b.i_add(int, None, div, pow2).expect("add to keep live");
    b.ret().expect("ret");
    b.end_function().expect("end");
    b.module().assemble()
}
...
fn build_signed_mask_then_shift_width_sized_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let mask = b.constant_bit32(int, 7);
    let band = b.bitwise_and(int, None, x, mask).expect("band");
    let shift = b.constant_bit32(int, 32);
    let _ = b
        .shift_right_arithmetic(int, None, band, shift)
        .expect("shr");
    b.ret().expect("ret");
...
fn build_mask_then_shift_width_sized_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mask_then_shift_u64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.constant_bit64(int, 0x1234);
    let mask = b.constant_bit64(int, (1u64 << 8) - 1); // 0xFF
    let band = b.bitwise_and(int, None, x, mask).expect("band");
    let shift = b.constant_bit64(int, 4);
    let _ = b.shift_right_logical(int, None, band, shift).expect("shr");
    b.ret().expect("ret");
    b.end_function().expect("end");
    b.module().assemble()
...
fn build_signed_mask_then_shift_u64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_band_full_mask_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.constant_bit32(int, 0xABCD_EF01);
...
fn build_bor_fold_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_bxor_complement_fold_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.constant_bit32(int, 0xA5A5_5A5A);
    let notx = b.not(int, None, x).expect("not x");
    let _ = b.bitwise_xor(int, None, x, notx).expect("xor complement");
    let _ = b
        .bitwise_xor(int, None, notx, x)
        .expect("xor complement commuted");
    b.ret().expect("ret");
    b.end_function().expect("end");
    b.module().assemble()
}

fn build_band_complement_fold_u64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.constant_bit64(int, 0xFFFF_FFFF_FFFFu64);
    let notx = b.not(int, None, x).expect("not x");
    let _ = b.bitwise_and(int, None, x, notx).expect("band complement");
    b.ret().expect("ret");
    b.end_function().expect("end");
    b.module().assemble()
}

fn build_band_complement_fold_u32_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let x = b.constant_bit32(int, 0xFFFF_FFFF);
    let notx = b.not(int, None, x).expect("not x");
    let _ = b.bitwise_and(int, None, x, notx).expect("band complement");
    b.ret().expect("ret");
    b.end_function().expect("end");
...
fn build_bnot_fold_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let c0 = b.constant_bit32(int, 0);
    let c3 = b.constant_bit32(int, 3);
    let _ = b.not(int, None, c0).expect("not zero");
    let _ = b.not(int, None, c3).expect("not const");
    b.ret().expect("ret");
    b.end_function().expect("end");
    b.module().assemble()
}

fn build_rotate_fold_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_rotate_fold_commuted_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_rotate_fold_signed_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_rotate_fold_signed_commuted_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_rotate_fold_u64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b
        ...
        .expect("function");
    let _ = b.begin_block(None).expect("block");
    let value = b.constant_bit64(int, 0x0123456789ABCDEF);
    let shift = b.constant_bit64(int, 16);
    let left = b.shift_left_logical(int, None, value, shift).expect("shl");
    let right_amount = b.constant_bit64(int, 48);
    let right = b
        .shift_right_logical(int, None, value, right_amount)
        .expect("shr");
    let _ = b
...
fn build_rotate_fold_u64_commuted_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_rotate_fold_signed_u64_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    let int = b.type_int(64, 1); // signed
    let func_ty = b.type_function(void, vec![]);
    let _ = b
        .begin_function(void, None, FunctionControl::NONE, func_ty)
        .expect("function");
...
fn build_rotate_fold_signed_u64_commuted_module() -> Vec<u32> {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mixed_unit_factor_add_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let x = b.function_parameter(int).expect("param x");
    let c3 = b.constant_bit32(int, 3);
    let mul = b.i_mul(int, None, x, c3).expect("mul");
    let add = b.i_add(int, None, x, mul).expect("add");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), add, x)
}

fn build_mixed_unit_factor_sub_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let x = b.function_parameter(int).expect("param x");
    let c7 = b.constant_bit32(int, 7);
    let mul = b.i_mul(int, None, c7, x).expect("mul");
    let sub = b.i_sub(int, None, mul, x).expect("sub");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), sub, x)
}
...
fn build_mul_div_constant_ratio_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let c3 = b.constant_bit32(int, 3);
    let mul = b.i_mul(int, None, c6, x).expect("mul");
    let div = b.u_div(int, None, mul, c3).expect("div");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), div, x)
}

fn build_mul_div_constant_ratio_signed_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let _ = b.begin_block(None).expect("block");
    let x = b.function_parameter(int).expect("param x");
    let c_neg6 = b.constant_bit32(int, (-6i32) as u32);
    let c_neg3 = b.constant_bit32(int, (-3i32) as u32);
    let mul = b.i_mul(int, None, c_neg6, x).expect("mul");
    let div = b.s_div(int, None, mul, c_neg3).expect("div");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), div, x)
}

fn build_nested_const_div_merge_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let c2 = b.constant_bit32(int, 2);
    let c3 = b.constant_bit32(int, 3);
    let div1 = b.u_div(int, None, x, c2).expect("div1");
    let div2 = b.u_div(int, None, div1, c3).expect("div2");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), div2, x)
}

fn build_mul_mod_same_const_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let rem = b.u_mod(int, None, mul, c5).expect("rem");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), rem)
}

fn build_mul_mod_divisible_const_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let rem = b.u_mod(int, None, mul, c3).expect("rem");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), rem)
}

fn build_mul_mod_non_divisible_const_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_srem_divisible_const_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let c_neg6 = b.constant_bit32(int, (-6i32) as u32);
    let c_neg3 = b.constant_bit32(int, (-3i32) as u32);
    let mul = b.i_mul(int, None, x, c_neg6).expect("mul");
...
fn build_mul_srem_divisible_const_pos_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_srem_non_divisible_const_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_srem_non_divisible_const_pos_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_srem_divisible_const_mixed_sign_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_srem_non_divisible_const_mixed_sign_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_div_non_divisible_ratio_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_div_constant_ratio_mixed_sign_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
    ...
    let c3 = b.constant_bit32(int, 3);
    let mul = b.i_mul(int, None, c_neg6, x).expect("mul");
    let div = b.s_div(int, None, mul, c3).expect("div");
    b.ret().expect("ret");
    b.end_function().expect("end");
    (b.module().assemble(), div, x)
}

fn build_mul_div_constant_ratio_pos_neg_div_module() -> (Vec<u32>, u32, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_div_non_divisible_ratio_mixed_sign_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
...
fn build_mul_div_non_divisible_ratio_pos_neg_div_module() -> (Vec<u32>, u32) {
    let mut b = Builder::new();
    b.capability(Capability::Shader);
    b.memory_model(AddressingModel::Logical, MemoryModel::Simple);
    let void = b.type_void();
