...
//! - `validation.egg`: SPIR-V validation rules (type checking, capability requirements)
...
const SPIRV_EGGLOG_PROGRAM: &str = concat!(
    ...
    "\n",
    include_str!("rules/validation.egg"),
);
...
mod tests {
    ...
    #[test]
    fn test_validation_type_propagation_const() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Const should be typed as 32-bit unsigned int
        egraph.parse_and_run_program(None, "(let c (Const 42))").unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 3 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (HasType c (TInt 32 0)))");
        assert!(check.is_ok(), "Const should have type (TInt 32 0)");
    }

    #[test]
    fn test_validation_type_propagation_const64() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Const64 should be typed as 64-bit unsigned int
        egraph.parse_and_run_program(None, "(let c (Const64 123456789012345))").unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 3 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (HasType c (TInt 64 0)))");
        assert!(check.is_ok(), "Const64 should have type (TInt 64 0)");
    }

    #[test]
    fn test_validation_type_propagation_add() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Add should preserve operand type
        egraph.parse_and_run_program(None, r#"
            (let a (Const 5))
            (let b (Const 10))
            (let sum (Add a b))
        ...
        let check = egraph.parse_and_run_program(None, "(check (HasType sum (TInt 32 0)))");
        assert!(check.is_ok(), "Add result should have operand type (TInt 32 0)");
    }

    #[test]
    fn test_validation_type_comparison_returns_bool() {
        let mut egraph = create_spirv_egraph().unwrap();
        ...
        let check = egraph.parse_and_run_program(None, "(check (HasType cmp (TBool)))");
        assert!(check.is_ok(), "Eq result should have type TBool");
    }

    #[test]
    fn test_validation_type_propagation_vector() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Vec2 should have vector type
        egraph.parse_and_run_program(None, r#"
            (let x (Const 1))
            (let y (Const 2))
            (let v (Vec2 x y))
        ...
        let check = egraph.parse_and_run_program(None, "(check (HasType v (TVec (TInt 32 0) 2)))");
        assert!(check.is_ok(), "Vec2 should have type (TVec (TInt 32 0) 2)");
    }

    #[test]
    fn test_validation_type_vec_extract() {
        let mut egraph = create_spirv_egraph().unwrap();

        // VecExtract should return element type
        egraph.parse_and_run_program(None, r#"
            (let x (Const 1))
            (let y (Const 2))
            (let v (Vec2 x y))
            (let elem (VecExtract v 0))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (HasType elem (TInt 32 0)))");
        assert!(check.is_ok(), "VecExtract should have element type (TInt 32 0)");
    }

    #[test]
    fn test_validation_division_by_zero() {
        let mut egraph = create_spirv_egraph().unwrap();
        ...
        let check = egraph.parse_and_run_program(None, "(check (DivisionByZero div))");
        assert!(check.is_ok(), "UDiv by constant 0 should be flagged as DivisionByZero");
    }

    #[test]
    fn test_validation_invalid_shift_amount() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Shift by >= 32 for 32-bit value should be flagged
        egraph.parse_and_run_program(None, r#"
            (let val (Const 1))
            (let shift (Const 32))
            (let result (Shl val shift))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (InvalidShiftAmount result))");
        assert!(check.is_ok(), "Shift by 32 should be flagged as InvalidShiftAmount");
    }

    #[test]
    fn test_validation_derivative_requires_uniform_cf() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Derivative operations should require uniform control flow
        egraph.parse_and_run_program(None, r#"
            (let x (Sym "x"))
            (let deriv (DPdx x))
        "#).unwrap();
        ...
        let check = egraph.parse_and_run_program(None, "(check (RequiresUniformControlFlow deriv))");
        assert!(check.is_ok(), "DPdx should require uniform control flow");
    }

    #[test]
    fn test_validation_64bit_requires_capability() {
        let mut egraph = create_spirv_egraph().unwrap();

        // 64-bit integers should require Int64 capability
        egraph.parse_and_run_program(None, "(let c (Const64 12345))").unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (RequiresCapability c (CapInt64)))");
        assert!(check.is_ok(), "Const64 should require CapInt64 capability");
    }

    #[test]
    fn test_validation_fine_derivative_requires_capability() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Fine derivatives require DerivativeControl capability
        egraph.parse_and_run_program(None, r#"
            (let x (Sym "x"))
            (let deriv (DPdxFine x))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (RequiresCapability deriv (CapDerivativeControl)))");
        assert!(check.is_ok(), "DPdxFine should require DerivativeControl capability");
    }

    #[test]
    fn test_validation_spec_const_compatible() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Spec constants and regular constants should be spec constant compatible
        egraph.parse_and_run_program(None, r#"
            (let sc (SpecConst 0 42))
            (let c (Const 10))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check1 = egraph.parse_and_run_program(None, "(check (IsSpecConstantCompatible sc))");
        assert!(check1.is_ok(), "SpecConst should be spec constant compatible");

        let check2 = egraph.parse_and_run_program(None, "(check (IsSpecConstantCompatible c))");
        assert!(check2.is_ok(), "Const should be spec constant compatible");
    }

    #[test]
    fn test_validation_spec_op_propagates_compatibility() {
        let mut egraph = create_spirv_egraph().unwrap();

        // SpecAdd of two compatible values should also be compatible
        egraph.parse_and_run_program(None, r#"
            (let a (SpecConst 0 10))
            (let b (Const 5))
            (let sum (SpecAdd a b))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (IsSpecConstantCompatible sum))");
        assert!(check.is_ok(), "SpecAdd of compatible values should be compatible");

        let check2 = egraph.parse_and_run_program(None, "(check (ValidSpecOp sum))");
        assert!(check2.is_ok(), "SpecAdd of compatible values should be valid");
    }

    #[test]
    fn test_validation_int_width_relation() {
        let mut egraph = create_spirv_egraph().unwrap();

        // IntWidth relation should extract bit width
        egraph.parse_and_run_program(None, r#"
            (let c32 (Const 42))
            (let c64 (Const64 123))
        "#).unwrap();
        ...
        let check = egraph.parse_and_run_program(None, "(check (IntWidth c32 32))");
        assert!(check.is_ok(), "Const should have IntWidth 32");

        let check = egraph.parse_and_run_program(None, "(check (IntWidth c64 64))");
        assert!(check.is_ok(), "Const64 should have IntWidth 64");
    }

    #[test]
    fn test_validation_is_bool_relation() {
        let mut egraph = create_spirv_egraph().unwrap();

        // IsBool relation for comparison results
        egraph.parse_and_run_program(None, r#"
            (let a (Const 5))
            (let b (Const 10))
            (let cmp (Eq a b))
        ...
        let check = egraph.parse_and_run_program(None, "(check (IsBool cmp))");
        assert!(check.is_ok(), "Comparison result should satisfy IsBool");
    }

    #[test]
    fn test_validation_is_vector_relation() {
        let mut egraph = create_spirv_egraph().unwrap();

        // IsVector relation for vector types
        egraph.parse_and_run_program(None, r#"
            (let x (Const 1))
            (let v (Vec3 x x x))
        "#).unwrap();
        ...
        let check = egraph.parse_and_run_program(None, "(check (IsVector v))");
        assert!(check.is_ok(), "Vec3 should satisfy IsVector");
    }

    #[test]
    fn test_validation_vec_size_relation() {
        let mut egraph = create_spirv_egraph().unwrap();

        // VecSize relation should extract component count
        egraph.parse_and_run_program(None, r#"
            (let x (Const 1))
            (let v2 (Vec2 x x))
            (let v4 (Vec4 x x x x))
        ...
        let check = egraph.parse_and_run_program(None, "(check (VecSize v2 2))");
        assert!(check.is_ok(), "Vec2 should have VecSize 2");

        let check = egraph.parse_and_run_program(None, "(check (VecSize v4 4))");
        assert!(check.is_ok(), "Vec4 should have VecSize 4");
    }

    #[test]
    fn test_validation_sdiv_by_zero() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Signed division by zero
        egraph.parse_and_run_program(None, r#"
            (let a (Const 42))
            (let div (SDiv a (Const 0)))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (DivisionByZero div))");
        assert!(check.is_ok(), "SDiv by constant 0 should be flagged as DivisionByZero");
    }

    #[test]
    fn test_validation_smod_by_zero() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Modulo by zero
        egraph.parse_and_run_program(None, r#"
            (let a (Const 42))
            (let m (SMod a (Const 0)))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (DivisionByZero m))");
        assert!(check.is_ok(), "SMod by constant 0 should be flagged as DivisionByZero");
    }

    #[test]
    fn test_validation_subgroup_requires_capability() {
        let mut egraph = create_spirv_egraph().unwrap();

        // Subgroup operations require capabilities
        egraph.parse_and_run_program(None, "(let e (GroupElect))").unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (RequiresCapability e (CapGroupNonUniform)))");
        assert!(check.is_ok(), "GroupElect should require CapGroupNonUniform");
    }

    #[test]
    fn test_validation_group_all_requires_vote_capability() {
        let mut egraph = create_spirv_egraph().unwrap();

        egraph.parse_and_run_program(None, r#"
            (let pred (Const 1))
            (let all (GroupAll pred))
        "#).unwrap();
        egraph.parse_and_run_program(None, "(run-schedule (repeat 5 (run)))").unwrap();

        let check = egraph.parse_and_run_program(None, "(check (RequiresCapability all (CapGroupNonUniformVote)))");
        assert!(check.is_ok(), "GroupAll should require CapGroupNonUniformVote");
    }
